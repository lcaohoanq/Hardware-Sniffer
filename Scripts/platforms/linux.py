...
class LinuxHardwareInfo:
    ...
    def read_and_decode(self, file_path, default=""):
        """Helper method to read a file and decode bytes to string safely."""
        try:
            content = self.utils.read_file(file_path)
    ...
    def clean_bytes_from_dict(self, obj):
        """Recursively convert all bytes objects to strings in a data structure."""
        if isinstance(obj, bytes):
            return obj.decode('utf-8', errors='ignore')
        elif isinstance(obj, dict):
            return {key: self.clean_bytes_from_dict(value) for key, value in obj.items()}
        elif isinstance(obj, list):
            return [self.clean_bytes_from_dict(item) for item in obj]
        elif isinstance(obj, tuple):
            return tuple(self.clean_bytes_from_dict(item) for item in obj)
        else:
            return obj
        ...
        for device_slot_name in os.listdir(PCI_DEVICES_PATH):
            ...
            for property_name in os.listdir(device_dir):
                ...
                if property_name == "vendor":
                    vendor_id = self.format_value(self.read_and_decode(device_property_path))
                elif property_name == "device":
                    device_id = self.format_value(self.read_and_decode(device_property_path))
                elif property_name == "subsystem_vendor":
                    subsystem_vendor_id = self.format_value(self.read_and_decode(device_property_path))
                elif property_name == "subsystem_device":
                    subsystem_device_id = self.format_value(self.read_and_decode(device_property_path))
    ...
    def motherboard(self):
        manufacturer = self.format_value(self.read_and_decode("/sys/class/dmi/id/sys_vendor")).split(" ")[0]
        model = self.format_value(self.read_and_decode("/sys/class/dmi/id/product_name"))

        board_manufacturer = self.format_value(self.read_and_decode("/sys/class/dmi/id/board_vendor")).split(" ")[0]
        board_model = self.format_value(self.read_and_decode("/sys/class/dmi/id/board_name"))
        ...
        system_platform = self.read_and_decode("/sys/class/dmi/id/chassis_type", "Unspecified")

        try:
            system_platform = int(self.read_and_decode("/sys/class/dmi/id/chassis_type"))
    ...
    def bios(self):
        ...
        bios_info["Version"] = self.format_value(self.read_and_decode("/sys/class/dmi/id/bios_version")) or "Unknown"
        bios_info["Release Date"] = self.format_value(self.read_and_decode("/sys/class/dmi/id/bios_date")) or "Unknown"
        ...
        bios_info["Secure Boot"] = "Enabled" if self.format_value(self.read_and_decode("/sys/firmware/efi/efivars/SecureBoot-8be4df61-93ca-11d2-aa0d-00e098032b8c", "0"), "int") else "Disabled"
    ...
    def cpu(self):
        try:
            cpu_cores = self.read_and_decode("/proc/cpuinfo").split("\n\n")
        except:
            cpu_cores = []
    ...
    def gpu(self):
        ...
        for graphics_device, _ in graphics_devices:
            ...
            uevent = self.format_value(self.read_and_decode(os.path.join(device_dir, "device", "uevent"), "\n"))
                ...
                for edid_path, _ in self.utils.find_matching_paths(gpu_dir, name_filter="edid", type_filter="file"):
                    ...
                    for property_name in os.listdir(monitor_dir):
                        ...
                        if property_name == "status":
                            connected = "connected" == self.format_value(self.read_and_decode(monitor_property_path))
                            
                            if not connected:
                                break
                        elif property_name == "modes":
                            for mode in self.format_value(self.read_and_decode(monitor_property_path, "\n")).splitlines():
                                try:
    ...
    def sound(self):
        ...
        for sound_card, _ in sound_cards:
            ...
            for property_name in os.listdir(device_dir):
                ...
                if property_name == "vendor":
                    vendor_id = self.format_value(self.read_and_decode(card_property_path))
                elif property_name == "device":
                    device_id = self.format_value(self.read_and_decode(card_property_path))
            ...
            for sound_device_dir, _ in sound_device_dirs:
                ...
                for property_name in os.listdir(os.path.join(device_dir, sound_device_dir)):
                    ...
                    if "chip_name" == property_name:
                        codec_name = self.format_value(self.read_and_decode(codec_property_path))
                    elif "vendor_name" == property_name:
                        codec_vendor = self.format_value(self.read_and_decode(codec_property_path))
                    elif "vendor_id" == property_name:
                        codec_id_raw = self.format_value(self.read_and_decode(codec_property_path))
                        if codec_id_raw and len(codec_id_raw) > 2:
                            codec_id = codec_id_raw[2:].upper()
                            codec_id = "{}-{}".format(codec_id[:4], codec_id[4:]) if len(codec_id) >= 4 else codec_id
                    elif "subsystem_id" == property_name:
                        subsystem_id_raw = self.format_value(self.read_and_decode(codec_property_path))
                        if subsystem_id_raw and len(subsystem_id_raw) > 2:
                            subsystem_id = subsystem_id_raw[2:].upper()
                    elif "modalias" == property_name:
                        modalias = self.read_and_decode(codec_property_path)
                        if modalias and ":" in modalias:
                            bus_type = self.format_value(modalias.split(":")[0].upper())
    ...
    def storage_controllers(self):
        ...
        for device in self.devices_by_class.get("Non-Volatile memory controller", []) + self.devices_by_class.get("SATA controller", []):
            ...
            for model_path, _ in model_paths:
                try:
                    disk_drive_names.append(self.format_value(self.read_and_decode(os.path.join(device_dir, model_path))))
                except:
                    pass
    ...
    def bluetooth(self):
        ...
        for device in os.listdir(BLUETOOTH_DEVICE_PATH):
            uevent = self.format_value(self.read_and_decode(os.path.join(BLUETOOTH_DEVICE_PATH, device, "device", "uevent"), "\n"))
    ...
    def sd_controller(self):
        ...
        for device in os.listdir(MMC_DEVICE_PATH):
            ...
            for physical_node, _ in physical_nodes:
                uevent = self.format_value(self.read_and_decode(os.path.join(device_dir, physical_node, "uevent"), "\n"))
                ...
                if "firmware_node" in property_name:
                    for firmware_node_property in os.listdir(device_property_path):
                        ...
                        if "description" == firmware_node_property:
                            device_description = self.format_value(self.read_and_decode(firmware_node_property_path))
                        elif "hid" == firmware_node_property:
                            hid = self.format_value(self.read_and_decode(firmware_node_property_path))
                elif "modalias" == property_name:
                    bus_type = self.format_value(self.read_and_decode(device_property_path)).split(":")[0].upper()
    ...
    def hardware_collector(self):
        ...
        # Clean any remaining bytes objects from the result
        self.result = self.clean_bytes_from_dict(self.result)
